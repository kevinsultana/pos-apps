...
export default function MasterSatuanEdit({ navigation, route }) {
  const { unit, onUpdate, onDelete } = route.params;
  ...
  const handleSave = () => {
    if (!canSave) return;
    onUpdate?.({ ...unit, name: name.trim(), symbol: symbol.trim() });
    if (Platform.OS === 'android') {
      ToastAndroid.show('Satuan berhasil diperbarui', ToastAndroid.SHORT);
    }
    navigation.goBack();
  };
  ...
  const handleDelete = () => {
    onDelete?.(unit.id);
    if (Platform.OS === 'android') {
      ToastAndroid.show('Satuan berhasil dihapus', ToastAndroid.SHORT);
    }
    navigation.goBack();
  };
    ...
    <AppLayout navigation={navigation} route={route}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Edit Satuan</Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity style={styles.deleteBtn} onPress={handleDelete}>
            ...
            <Text style={styles.deleteText}>Hapus Satuan</Text>
          </TouchableOpacity>
        </View>
      </View>
    </AppLayout>
