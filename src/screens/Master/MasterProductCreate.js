...
export default function MasterProductCreate({ navigation, route }) {
  ...
  const canSave = useMemo(() => {
    return name.trim() && sku.trim() && barcode.trim();
  }, [name, sku, barcode]);
  ...
  const onSave = () => {
    ...
    if (!payload.name) return showToast('Nama produk wajib diisi');
    if (!payload.sku) return showToast('SKU wajib diisi');
    if (!payload.barcode) return showToast('Barcode wajib diisi');
